...
public class HtmlTreeBuilder extends TreeBuilder {
    ...
    private void clearStackToContext(String... strArr) {
        ...
        while (size >= 0) {
            ...
            if (!StringUtil.m50211in(element.nodeName(), strArr) && !element.nodeName().equals("html")) {
    ...
    public void generateImpliedEndTags(String str) {
        while (str != null && !currentElement().nodeName().equals(str) && StringUtil.m50211in(currentElement().nodeName(), TagSearchEndTags)) {
            pop();
        }
    }
    ...
    public boolean inSelectScope(String str) {
        for (int size = this.stack.size() - 1; size >= 0; size--) {
            ...
            if (!StringUtil.m50211in(nodeName, TagSearchSelectScope)) {
                return false;
            }
        }
    ...
    public boolean isSpecial(Element element) {
        return StringUtil.m50211in(element.nodeName(), TagSearchSpecial);
    }
    ...
    public List<Node> parseFragment(String str, Element element, String str2, ParseErrorList parseErrorList, ParseSettings parseSettings) {
        ...
        if (element != null) {
            ...
            if (StringUtil.m50211in(tagName, "title", "textarea")) {
                this.tokeniser.transition(TokeniserState.Rcdata);
            } else if (StringUtil.m50211in(tagName, "iframe", "noembed", "noframes", "style", "xmp")) {
    ...
    private boolean inSpecificScope(String[] strArr, String[] strArr2, String[] strArr3) {
        for (int size = this.stack.size() - 1; size >= 0; size--) {
            String nodeName = this.stack.get(size).nodeName();
            if (StringUtil.m50211in(nodeName, strArr)) {
                return true;
            }
            if (StringUtil.m50211in(nodeName, strArr2)) {
                return false;
            }
            if (strArr3 != null && StringUtil.m50211in(nodeName, strArr3)) {
                return false;
            }
        }
        Validate.fail("Should not be reachable");
    ...
    public void popStackToClose(String... strArr) {
        ...
        while (size >= 0) {
            this.stack.remove(size);
            if (!StringUtil.m50211in(this.stack.get(size).nodeName(), strArr)) {
                size--;
            } else {
                return;
            }
